 ...
 * - Text: FIXED position across all aspect ratios
...
// Apply smart aspect ratio, overlay, and add text overlays
async function applySmartAspectRatioOverlayAndText(videoPath, quote, author, watermark, captions, overlay, outputPath, maxDuration) {
  return new Promise(async (resolve, reject) => {
    try {
      console.log('📐 Applying smart aspect ratio management and text overlays...');
      ...
      if (overlay) {
        ...
        if (fs.existsSync(overlayPath)) {
          ...
          let filterComplex;
          if (textFilter) {
            filterComplex = `[0:v]${baseVideoFilter}[scaled];[scaled]split[s1][s2];[s2]eq=brightness=-0.3[darkened];[1:v]scale=1080:1920[mask];[darkened][s1][mask]maskedmerge[overlaid];[overlaid]${textFilter}`;
          } else {
            filterComplex = `[0:v]${baseVideoFilter}[scaled];[scaled]split[s1][s2];[s2]eq=brightness=-0.3[darkened];[1:v]scale=1080:1920[mask];[darkened][s1][mask]maskedmerge[overlaid]`;
          }
...
// Main function
async function createVideoVid13(audioPath, quote, author, watermark, clips, captions, outputPath, overlay = false) {
  console.log('🚀 Starting Vid-1.3 with smart aspect ratio management and overlay support...');
  console.log(`📊 Input: ${clips.length} clips, Audio: ${path.basename(audioPath)}, Overlay: ${overlay}`);
  ...
  try {
    ...
    // Apply smart aspect ratio, overlay, and text (with captions support)
    const videoWithTextPath = path.join('temp', `${sessionId}-with-text.mp4`);
    await applySmartAspectRatioOverlayAndText(concatenatedVideoPath, quote, author, watermark, captions, overlay, videoWithTextPath, finalDuration);
    ...
    console.log('🎉 Vid-1.3 video created successfully with smart aspect ratio!');
    return outputPath;

  } catch (error) {
    console.error('❌ Vid-1.3 creation failed:', error.message);
    throw error;
  }
}
